            ...
            <CardContent className="space-y-6 pt-0">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  // Use the nested handler for popup.text
              ...
              {/* --- START: ADDED PDF Link Input --- */}
              <div className="space-y-2">
                <Label htmlFor={`pdf-link-${editingNode.id}`}>PDF Link (Optional)</Label>
                <Input
                  id={`pdf-link-${editingNode.id}`}
                  type="text" // Using text type for flexibility, could use 'url'
                  value={editingNode.popup?.pdf_link || ""} // Bind to popup.pdf_link, default to empty string
                  // Use the nested handler for popup.pdf_link
                  onChange={(e) => handleNestedEditingChange(["popup", "pdf_link"], e.target.value)}
                  placeholder="https://example.com/path/to/exercise.pdf"
                  className="text-sm" // Match styling if needed
                />
                 <p className="text-xs text-muted-foreground">
                    Link to an external PDF document related to this node. Used in the main app.
                 </p>
              </div>
              {/* --- END: ADDED PDF Link Input --- */}
